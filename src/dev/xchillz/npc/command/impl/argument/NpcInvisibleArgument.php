...
final class NpcInvisibleArgument extends Argument {
    ...
    public function onPlayerExecute(
        ...
        if (!isset($args[0])) {
            $player->sendMessage($this->getMessagesConfig()->getMessage('npc.set.invisible.usage', []));
            return;
        }
        ...
        $player->sendMessage($this->getMessagesConfig()->getMessage('npc.set.visible.success', ['id' => $npcId]));
    }
